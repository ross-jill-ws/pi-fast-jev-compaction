// Vendored from fast-jev-compaction (MIT, https://github.com/tamaratran/fast-jev-compaction), src/compact.ts.
// pi changes: AbortSignal threaded through to the asker, Jev usage accumulated,
// the paired calls returned with the result, and the truncation note renamed.
...
interface BatchOutcome {
  ...
  inputTokens: number;
  outputTokens: number;
  /** The raw request and response, for debug bundles. */
...
export function truncatedResultText(text: string, isError: boolean, headChars: number): string {
  ...
  return `${head}[pi-fast-jev-compaction truncated ${text.length - headChars} chars of this tool result${
    isError ? ' (error)' : ''
...
/** Everything sent to and received from Jev during one compaction, for debugging. */
export interface JevTrace {
  state: CompactionState | undefined;
  requests: { questions: JevQuestions; answers: Record<string, unknown> }[];
}

export interface CompactRun extends CompactResult {
  trace: JevTrace;
}
...
export async function compact(
  ...
  const trace: JevTrace = { state: undefined, requests: [] };
  ...
  if (candidates.length > 0) {
    ...
    for (const outcome of answered) {
      ...
      jevInputTokens += outcome.inputTokens;
      jevOutputTokens += outcome.outputTokens;
      trace.requests.push({ questions: outcome.questions, answers: outcome.rawAnswers });
    }
  }
